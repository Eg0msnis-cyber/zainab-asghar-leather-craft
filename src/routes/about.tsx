...
export const Route = createFileRoute("/about")({
  head: () => ({ meta: [
    { title: "About Zainab Asghar — Sialkot Leather Craft" },
...
function AboutPage() { const { lang } = useLanguage(); const t = copy[lang]; const icons=[Scissors,Check,ShieldCheck]; return <>
  ...
  <section className="border-y border-border bg-surface"><div className="page-shell grid gap-px bg-border md:grid-cols-3">{t.craftPillars.map(([title,desc],i)=>{const Icon=icons[i]; return <article key={title} className="bg-surface p-8 md:py-14"><span className="eyebrow">0{i+1}</span><Icon className="my-8 text-primary"/><h3 className="font-display text-3xl">{title}</h3><p className="mt-4 text-sm leading-7 text-muted-foreground">{desc}</p></article>})}</div></section>
