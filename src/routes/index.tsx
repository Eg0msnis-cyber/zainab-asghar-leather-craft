...
function HomePage() {
  ...
  return <>
    ...
    <section className="section-pad"><div className="page-shell"><SectionHead num="01" title={t.featured} sub={t.featuredSub}/><div className="mt-12 grid gap-5 md:grid-cols-3">{(["jackets","bags","shoes"] as const).map((cat)=><Link to="/products" key={cat} className="group relative aspect-[4/5] overflow-hidden bg-card"><img src={categoryImages[cat].url} alt={categoryNames[lang][cat]} className="size-full object-cover transition-transform duration-700 group-hover:scale-105"/><div className="image-shade absolute inset-0"/><div className="absolute inset-x-0 bottom-0 p-6"><span className="eyebrow">{t.from} {cat === "jackets" ? "4,500" : "2,500"} PKR</span><h3 className="mt-2 font-display text-3xl">{categoryNames[lang][cat]}</h3></div></Link>)}</div></div></section>
